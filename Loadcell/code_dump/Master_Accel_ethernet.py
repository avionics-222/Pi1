        ...
        try:
            ...
            if message.startswith("ACCEL"):
                ...
                if len(parts) == 6:
                    ...
                    queue.put(("ACCEL", (float(x), float(y), float(z))))
...
if __name__ == "__main__":
   ...
   # main(23,24,270,-979799.5,do_calib)
    ...
    csv_writer.writerow([datetime.now().strftime("%H:%M:%S.%f")[:-3]])
    ...
    if accel_state == 1:
        headers += ['Accel_Location', 'Accel_X', 'Accel_Y', 'Accel_Z']
    ...
    try:
        ...
        if accel_state == 1:
            accel_thread = multiprocessing.Process(target=udp_listener, args=(queue, "RPi2"))
        ...
        while True:
           ...
           ##################### Total weight experienced by all the 4 loadcells #################################
            ...
            if esc_data_state == 1 and load_cell_state == 1:
                ...
                row.extend(ESC_data_output)
            if accel_state == 1:
                row.extend([accel_location,round(accel_data["x"], 3),round(accel_data["y"], 3),round(accel_data["z"], 3)])
