...
if __name__ == "__main__":
    ...
    parser.add_argument('--accel', action='store_true', help='Enable accel data collection')
    parser.add_argument('--accel_location', type=str, default="-", help='Location identifier for accel data')
   ...
   # main(23,24,270,-979799.5,do_calib)
    ...
    try:
        ...
        if accel_state == 1:
            ...
            udp_process = multiprocessing.Process(target=udp_listener, args=(udp_queue,))
            udp_process.start()
            process_list.append(udp_process)

            udp_data = {"location": "-","timestamp": "-","x": 0.0,"y": 0.0,"z": 0.0}

        while True:
            time.sleep(can_bus_timeout)
            while not udp_queue.empty():
                try:
                    key, data = udp_queue.get()
                    if key == "ACCEL":
                        udp_data = data
                except Exception as e:
            ...
            if esc_data_state == 1 and load_cell_state == 1:
                ...
                row.extend(ESC_data_output)
            if accel_state == 1:
                row.extend([udp_data["timestamp"], udp_data["location"], udp_data["x"], udp_data["y"], udp_data["z"]])
