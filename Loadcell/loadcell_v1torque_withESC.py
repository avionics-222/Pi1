...
global nominal_rpm, kt_value, can_bus_timeout, multi_rate, CAN_CYCLIC_RATE, num_esc_param, offset_loss_20

offset_loss_20 = 1            #Compensation for 20.2% loss on torque setup
...
def loadcell_worker(dout, sck, load_cell_label, offset, queue, index):
    ...
    try:
        while True:
            ...
            queue.put((index, weight_out * offset_loss_20)) # Abs for torque values 
...
#                        print("##################",torque_raw)
                   ...
                   #$ print(bin_data)
                    ...
                    '''
                    first_14_bits = bin_data[14:24]
                    second_14_bits = bin_data[28:42]
                    third_18_bits = bin_data[42:60]
                    fourth_18_bits = bin_data[56:74]
                    '''
...
if __name__ == "__main__":
    ...
    hardcoded_offsets = [-715552.5, -893601.5, -687848.0, 1740185.0, 0, 0, 0, 0]
    ...
    # Setup CSV file
    os.makedirs("logs_torque", exist_ok=True)
    csv_filename = f"logs_torque/{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    try:
        ...
        while True:
            ...
            total_weight = abs(sum(latest_weights[0:4])) 
            
            if load_cell_state == 1:
                print(f"Latest Weights: {latest_weights} | Total: {total_weight:.2f} Kg | Torque: {total_weight * 9.8 * 0.23: .3f} Nm")
            if esc_data_state == 1:
